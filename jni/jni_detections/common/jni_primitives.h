...
class JNI_Float : public JavaPeer {
 public:
  JNI_Float(JNIEnv *pEnv)
      : JavaPeer(pEnv, CLASSNAME_FLOAT, CONSTSIG_FLOAT) {
  }
};
...
class JNI_PointF : public JavaPeer {
 public:
  JNI_PointF(JNIEnv* pEnv)
      : JavaPeer(pEnv, CLASSNAME_POINTF, CONSTSIG_POINTF) {
...
class JNI_VisionDetRet {
 public:
  ...
  void setRect(JNIEnv* env, jobject& jDetRet, const int& left, const int& top, const int& right, const int& bottom) {
